...
describe('parseHeadingNotation', () => {
  ...
  it('marks *spans* as accent segments', () => {
    expect(parseHeadingNotation('See how teams *build on VLabs*')).toEqual([
      { kind: 'text', text: 'See how teams ' },
      { kind: 'accent', text: 'build on VLabs' },
    ]);
  });
  ...
  it('supports multiple accents', () => {
    expect(parseHeadingNotation('*Virtual labs* for teams that *move fast*')).toEqual([
      { kind: 'accent', text: 'Virtual labs' },
