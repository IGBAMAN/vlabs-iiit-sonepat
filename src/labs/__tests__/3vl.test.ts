...
describe("3VL — predicates", () => {
  it("isFullyDefined [1,0,1]", () =>
    expect(isFullyDefined([1, 0, 1])).toBe(true));
  it("!isFullyDefined [1,x,1]", () =>
    expect(isFullyDefined([1, "x", 1])).toBe(false));
...
describe("3VL — bitwise AND (3-valued rules)", () => {
  ...
  it("0 AND x = 0  (short-circuit)", () =>
    expect(sigAnd([0], ["x"])).toEqual([0]));
  it("x AND 0 = 0  (short-circuit)", () =>
    expect(sigAnd(["x"], [0])).toEqual([0]));
...
describe("3VL — bitwise OR (3-valued rules)", () => {
  ...
  it("1 OR x = 1  (short-circuit)", () =>
    expect(sigOr([1], ["x"])).toEqual([1]));
  it("x OR 1 = 1  (short-circuit)", () =>
    expect(sigOr(["x"], [1])).toEqual([1]));
...
describe("3VL — reduce operations", () => {
  ...
  it("AND-reduce [1,x,1] = x", () =>
    expect(reduceAnd([1, "x", 1])).toEqual(["x"]));
  it("AND-reduce [0,x,1] = 0 (short-circuit)", () =>
    expect(reduceAnd([0, "x", 1])).toEqual([0]));
  it("OR-reduce  [0,0,1] = 1", () => expect(reduceOr([0, 0, 1])).toEqual([1]));
  it("OR-reduce  [0,0,0] = 0", () => expect(reduceOr([0, 0, 0])).toEqual([0]));
  it("OR-reduce  [1,x,0] = 1 (short-circuit)", () =>
    expect(reduceOr([1, "x", 0])).toEqual([1]));
  it("XOR-reduce [1,1,0] = 0 (even parity)", () =>
    expect(reduceXor([1, 1, 0])).toEqual([0]));
  it("XOR-reduce [1,0,1] = 0 (even parity)", () =>
    expect(reduceXor([1, 0, 1])).toEqual([0]));
  it("XOR-reduce [1,0,0] = 1 (odd parity)", () =>
    expect(reduceXor([1, 0, 0])).toEqual([1]));
