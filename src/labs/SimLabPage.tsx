...
function SectionContent({
  ...
  if (sec.type === 'simulation') {
    ...
    if (!SimComp) return <p className="text-[var(--ink)] font-sans text-sm leading-[1.75] m-0">Unknown simulation type: {sec.simType}</p>;
    return (
  ...
  if (section.type === 'text') {
    return (
      <div className="bg-white rounded-[calc(var(--radius-base)*3)] shadow-[0_2px_16px_rgba(0,0,0,0.08),0_0_0_1px_rgba(0,0,0,0.04)] max-w-[680px]" style={{ padding: 'calc(var(--spacing-base) * 6) calc(var(--spacing-base) * 7)' }}>
        <h2 className="text-[var(--ink)] font-sans text-base font-medium mb-[calc(var(--spacing-base)*4)] mt-0">{section.title}</h2>
    ...
    return (
      <div className="bg-white rounded-[calc(var(--radius-base)*3)] shadow-[0_2px_16px_rgba(0,0,0,0.08),0_0_0_1px_rgba(0,0,0,0.04)] max-w-[680px]" style={{ padding: 'calc(var(--spacing-base) * 6) calc(var(--spacing-base) * 7)' }}>
        ...
        {section.items.map((item, i) => (
          <div key={i} className="flex gap-[calc(var(--spacing-base)*3)] mb-[calc(var(--spacing-base)*2)]">
            ...
            <div>
              ...
              {item.specification && <> — <span className="text-[var(--ink-muted)] font-sans text-[12px]"><MathText text={item.specification} /></span></>}
  ...
  if (section.type === 'procedure') {
    ...
    return (
      <div className="bg-white rounded-[calc(var(--radius-base)*3)] shadow-[0_2px_16px_rgba(0,0,0,0.08),0_0_0_1px_rgba(0,0,0,0.04)] max-w-[680px]" style={{ padding: 'calc(var(--spacing-base) * 6) calc(var(--spacing-base) * 7)' }}>
        ...
        {step ? (
          step.body.split('\n').map((line, i) => (
            <p key={i} className="text-[var(--ink)] font-sans text-sm leading-[1.75] m-0 [&+&]:mt-[calc(var(--spacing-base)*3)]">
              <MathText text={line} />
            </p>
          ))
        ) : (
          <p className="text-[var(--ink)] font-sans text-sm leading-[1.75] m-0">Select a step to begin.</p>
        )}
      </div>
    );
  }
          ...
          <table className="border-collapse font-sans text-[13px] mt-[calc(var(--spacing-base)*4)] w-full">
            <thead>
              <tr>{section.table.headers.map((h) => (
                <th key={h} className="border-b border-[var(--color-black-10)] text-[var(--ink-muted)] text-[11px] font-medium tracking-[0.04em] px-[calc(var(--spacing-base)*2)] py-[calc(var(--spacing-base)*1.5)] text-left uppercase">
                  <MathText text={h} />
                </th>
              ))}</tr>
            </thead>
            <tbody>
              {section.table.rows.map((row, ri) => (
                <tr key={ri}>{row.map((cell, ci) => (
                  <td key={ci} className="border-b border-[var(--color-black-10)] text-[var(--ink)] px-[calc(var(--spacing-base)*2)] py-[calc(var(--spacing-base)*1.5)]">
  ...
  return (
    <div className="flex h-dvh overflow-hidden bg-[var(--color-neutral)]">
      ...
      <div className="flex-1 min-w-0 overflow-hidden relative flex flex-col">
        <div className="flex-1 overflow-y-auto bg-[var(--color-neutral)]" style={{ padding: 'calc(var(--spacing-base) * 6)' }}>
          {activeSection && (
            <SectionContent
