...
**Important:** `components.ts` in an experiment folder is *not* the same as `src/components/`. The experiment file lists circuit instances; `src/components/` holds the shared render code.
...
export const M = {
  r1_p1: [resistorP1(5, 'c')[0], TOP_Y + 0.55, resistorP1(5, 'c')[2]] as [number, number, number],
};
```
