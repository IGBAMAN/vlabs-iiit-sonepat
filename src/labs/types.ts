...
export type WireColor =
  "red" | "black" | "yellow" | "green" | "blue" | "orange" | "white" | "purple";
...
export type ComponentInstance =
  ...
  | { id: string; type: "long-breadboard" }
  ...
  | {
      id: string;
      type: "zero-extend";
      extend: { input: number; output: number };
      mountedAt: MountPoint;
    }
  | {
      id: string;
  ...
  | {
      id: string;
      type: "bus-slice";
      slice: { first: number; count: number; total: number };
      mountedAt: MountPoint;
    }
...
export type TruthTableRow = {
  inputs: Record<string, 0 | 1 | number>;
  outputs: Record<string, 0 | 1 | number>;
};
