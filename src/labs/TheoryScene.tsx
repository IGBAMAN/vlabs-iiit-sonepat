...
function addText(
  ...
  ctx.textAlign =
    anchor === "left" ? "left" : anchor === "right" ? "right" : "center";
  ctx.fillText(
    text,
    anchor === "left" ? 12 : anchor === "right" ? cw - 12 : cw / 2,
    ch / 2,
  );
...
function buildResistor(
  ...
  if (label) addText(parent, label, cx, cy + rh + 0.16, 0.1, "#555");
}
...
function buildZener(parent: THREE.Object3D, cx: number, cy: number) {
  ...
  // Labels
  addText(parent, "A", cx - hh - leadLen / 2, cy + hh + 0.14, 0.1, "#888");
  addText(parent, "K", cx + hh + leadLen / 2, cy + hh + 0.14, 0.1, "#888");
  addText(parent, "Dz", cx, cy - hh - 0.22, 0.1, "#141414");
}
...
function buildBattery(
  ...
  // +/− labels
  addText(parent, "+", cx + 0.28, cy + 0.2, 0.12, "#141414");
  addText(parent, "−", cx + 0.28, cy - 0.2, 0.12, "#141414");
  if (label) addText(parent, label, cx - 0.32, cy, 0.1, "#555", "right");
}
...
function buildMeter(
  ...
  addText(parent, sym, cx, cy, 0.15, sym === "V" ? "#2563a8" : "#141414");
  if (label) addText(parent, label, cx, cy + METER_R + 0.22, 0.1, "#888");
}
...
function buildCurrentArrow(
  ...
  if (label)
    addText(parent, label, mx + uy * 0.22, my - ux * 0.22, 0.09, "#d63b2a");
}
...
function buildSchematic(spec: SchematicSpec, scene: THREE.Scene) {
  ...
  for (const el of spec.elements) {
    switch (el.type) {
      ...
      case "resistor":
        buildResistor(root, el.cx, el.cy, el.angle ?? 0, el.label);
        break;
      case "zener":
        buildZener(root, el.cx, el.cy);
        break;
      case "battery":
        buildBattery(root, el.cx, el.cy, el.label);
        break;
      case "meter":
        buildMeter(root, el.cx, el.cy, el.symbol, el.orient ?? "h", el.label);
        break;
      case "gnd":
        buildGnd(root, el.cx, el.cy);
        break;
      case "node":
        filledCircle(root, el.x, el.y, 0.045);
        break;
      case "label":
        addText(
          root,
        ...
        break;
      case "current":
        buildCurrentArrow(root, el.x1, el.y1, el.x2, el.y2, el.label);
        break;
    }
  }
...
export function TheoryScene({ spec }: Props) {
  ...
  useEffect(() => {
    ...
    const onMove = (e: PointerEvent) => {
      ...
      const scaleY =
        (camera.top - camera.bottom) / Math.max(canvas.clientHeight, 1);
      panX -= (e.clientX - lx) * scaleX;
      panY += (e.clientY - ly) * scaleY;
    ...
    const onWheel = (e: WheelEvent) => {
      ...
      zoomRef.current = Math.max(
        0.25,
        Math.min(6, zoomRef.current * (e.deltaY > 0 ? 0.88 : 1.14)),
      );
