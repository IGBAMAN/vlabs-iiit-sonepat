...
function updateLRU(lines: CacheLine[], usedIdx: number): CacheLine[] {
  ...
  return lines.map((line, i) => {
    ...
    if (line.lruOrder < usedOrder)
      return { ...line, lruOrder: line.lruOrder + 1 };
    return line;
  });
}
...
export function SimCacheAssociative({ description }: Props) {
  ...
  const handleAccess = useCallback(() => {
    ...
    if (isHit) {
      ...
      if (emptyIdx !== -1) {
        ...
        loadedIdx = cache.reduce(
          (maxI, l, i) => (l.lruOrder > cache[maxI].lruOrder ? i : maxI),
          0,
        );
        evicted = loadedIdx;
      }
  ...
  return (
    <div style={S.root}>
      <div style={S.title}>
        Fully-Associative Cache — 4 lines, LRU replacement
      </div>
      ...
      <div style={S.addrBreakdown}>
        <div style={S.field("#f06595")}>
          <span style={{ fontWeight: 700 }}>
            {curTag.toString(2).padStart(TAG_BITS, "0")}
          </span>
          <span style={{ fontSize: 10, marginTop: 2 }}>TAG ({TAG_BITS}b)</span>
        </div>
        <div
          style={{ alignSelf: "center", color: "#495057", padding: "0 4px" }}
        >
          |
        </div>
        <div style={S.field("#4dabf7")}>
          <span style={{ fontWeight: 700 }}>
            {curOffset.toString(2).padStart(OFFSET_BITS, "0")}
          </span>
          <span style={{ fontSize: 10, marginTop: 2 }}>
            OFFSET ({OFFSET_BITS}b)
          </span>
        </div>
        <div
          style={{
      ...
      <div style={S.controlRow}>
        ...
        <input
          ...
          maxLength={2}
        />
        <button style={S.btn("#4dabf7")} onClick={handleAccess}>
          ACCESS
        </button>
        ...
        {lastResult && (
          <span style={S.statusChip(lastResult.type)}>
            {lastResult.type.toUpperCase()} @ 0x
            {lastResult.addr.toString(16).toUpperCase().padStart(2, "0")}
          </span>
        )}
      </div>
          ...
          {cache.map((line, i) => {
            ...
            return (
              <tr key={i} style={S.tr(activeLineIdx === i, fl, isEvicted)}>
                <td style={S.td}>{i}</td>
                <td
                  style={{ ...S.td, color: line.valid ? "#51cf66" : "#495057" }}
                >
                  {line.valid ? "1" : "0"}
                </td>
                ...
                {line.data.map((b, di) => (
                  <td
                    ...
                    style={{
                      ...S.td,
                      color: line.valid ? "#e9ecef" : "#495057",
                    }}
                  >
                    {line.valid ? toHex2(b) : "--"}
                  </td>
                ...
                <td style={S.td}>
                  {isEvicted && (
                    <span style={{ color: "#ffd43b", fontSize: 11 }}>
                      ⟵ EVICTED
                    </span>
                  )}
      ...
      <div style={S.statsBar}>
        <span>
          Accesses: <span style={S.statVal("#e9ecef")}>{accesses}</span>
        </span>
        <span>
          Hits: <span style={S.statVal("#51cf66")}>{hits}</span>
        </span>
        <span>
          Misses: <span style={S.statVal("#ff6b6b")}>{accesses - hits}</span>
        </span>
        <span>
          Hit Rate:{" "}
          <span style={S.statVal("#4dabf7")}>
            {hitRate}
            {accesses > 0 ? "%" : ""}
          </span>
        </span>
