...
function makeInitialMemory(): Uint8Array {
  ...
  const romData = [
    0xff, 0x00, 0x3c, 0xa5, 0xde, 0xad, 0xbe, 0xef, 0x12, 0x34, 0x56, 0x78,
    0x9a, 0xbc, 0xde, 0xf0, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80,
...
export function SimMemory({ description }: Props) {
  ...
  const handleRead = useCallback(() => {
    ...
    if (addr === null || addr < 0 || addr > 255) {
      setError("Invalid address (00–FF)");
      return;
    }
    setError("");
  ...
  const handleWrite = useCallback(() => {
    ...
    if (data === null || data < 0 || data > 255) {
      setError("Invalid data (00–FF)");
      return;
    }
    if (addr <= ROM_END) {
      setError("ROM is read-only! Addresses 0x00–0x1F cannot be written.");
      return;
    }
    setError("");
    ...
    <div style={S.root}>
      ...
      <div style={S.controlRow}>
        ...
        <input
          ...
          maxLength={2}
        />
        <button style={S.btn("#4dabf7")} onClick={handleRead}>
          READ
        </button>
        <button style={S.btn("#51cf66")} onClick={handleWrite}>
          WRITE
        </button>
        ...
        <table style={S.gridTable}>
          ...
          <tbody>
            {rows.map((row, ri) => (
              <tr key={ri}>
                <td style={S.rowLabel}>
                  {(ri * 16).toString(16).toUpperCase().padStart(2, "0")}
                </td>
                {row.map((addr) => {
        ...
        <span>
          ...
          {lastRead ? (
            <span style={{ color: "#4dabf7" }}>
              {toHex2(lastRead.val)} from {toHex4(lastRead.addr)}
            </span>
          ) : (
        ...
        <span>
          ...
          {lastWrite ? (
            <span style={{ color: "#ffa94d" }}>
              {toHex2(lastWrite.val)} to {toHex4(lastWrite.addr)}
            </span>
          ) : (
            <span style={{ color: "#495057" }}>—</span>
          )}
