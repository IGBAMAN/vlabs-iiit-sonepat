...
function stepCPU(state: CPUState, lines: string[]): CPUState {
  if (state.halted)
    return {
      ...state,
      stageDetail: "CPU is halted. Press Reset.",
      stage: "HALTED",
    };
  ...
  if (pc >= lines.length) {
    return {
      ...
      halted: true,
      stage: "HALTED",
      stageDetail: "No more instructions",
      activeBlock: null,
    };
  }
  ...
  if (!parsed) {
    ...
    return {
      ...
      pc: pc + 1,
      stage: "DECODE",
      stageDetail: `DECODE: skipped blank line`,
      activeBlock: "ir",
    };
  }
  ...
  switch (op) {
    case "LOAD": {
      ...
      if (memRef !== null) {
        ...
        return {
          ...
          regs: newRegs,
          stage: "EXECUTE",
          stageDetail: `EXECUTE: LOAD ${dest} ← mem[${memRef}] = ${newRegs[dest]}`,
          activeBlock: "memory",
        };
      }
      ...
      if (imm !== null) {
        ...
        return {
          ...
          regs: newRegs,
          stage: "EXECUTE",
          stageDetail: `EXECUTE: LOAD ${dest} = ${imm}`,
          activeBlock: "regs",
        };
      }
      return {
        ...afterFetch,
        error: `Bad LOAD operand: ${src}`,
        stage: "ERROR",
        activeBlock: "ir",
      };
    }
    ...
    case "ADD": {
      ...
      if (!isReg(rd) || !isReg(rs1) || !isReg(rs2)) {
        return {
          ...afterFetch,
          error: `ADD requires 3 registers`,
          stage: "ERROR",
          activeBlock: "ir",
        };
      }
      ...
      return {
        ...
        regs: newRegs,
        stage: "EXECUTE",
        stageDetail: `EXECUTE: ADD ${rd} = ${state.regs[rs1]} + ${state.regs[rs2]} = ${newRegs[rd]}`,
        activeBlock: "alu",
      };
    }
    ...
    case "STORE": {
      ...
      if (addr === null)
        return {
          ...afterFetch,
          error: `Bad STORE address: ${addrStr}`,
          stage: "ERROR",
          activeBlock: "ir",
        };
      ...
      return {
        ...
        memory: newMem,
        stage: "EXECUTE",
        stageDetail: `EXECUTE: STORE mem[${addr}] = ${state.regs[src]} (from ${src})`,
        activeBlock: "memory",
      };
    }
    ...
    case "JUMP": {
      ...
      return {
        ...
        pc: target,
        stage: "EXECUTE",
        stageDetail: `EXECUTE: JUMP → line ${target}`,
        activeBlock: "pc",
      };
    }
    ...
    case "HALT":
      return {
        ...
        halted: true,
        stage: "HALTED",
        stageDetail: "EXECUTE: HALT — CPU stopped",
        activeBlock: null,
      };
...
const S = {
  ...
  progLine: (active: boolean, halted: boolean): React.CSSProperties => ({
    ...
    borderLeft: active
      ? `3px solid ${halted ? "#ffa94d" : "#4dabf7"}`
...
export function SimCPU({ description }: Props) {
  ...
  const { pc, ir, regs, memory, stage, stageDetail, activeBlock, halted } =
    cpuState;
  ...
  return (
    <div style={S.root}>
      ...
      <div style={S.controlRow}>
        <button style={S.btn("#4dabf7")} onClick={handleStep} disabled={halted}>
          ▶ Step
        </button>
        <button style={S.btn("#51cf66")} onClick={handleRun} disabled={halted}>
          ▶▶ Run
        </button>
        <button style={S.btn("#495057")} onClick={handleReset}>
          ↺ Reset
        </button>
        <button
          style={{
            ...S.btn(editing ? "#ffa94d" : "#343a40"),
            color: editing ? "#1a1a2e" : "#ccc",
          }}
            ...
            <div style={S.progLines}>
              {displayLines.map((line, i) => (
                <div key={i} style={S.progLine(i === pc, halted)}>
                  <span
                    style={{ color: "#495057", fontSize: 10, marginRight: 8 }}
                  >
          ...
          <div style={S.block(activeBlock === "regs", "#51cf66")}>
            ...
            {REGS.map((r) => (
              <div key={r} style={S.regRow}>
                ...
                <span style={S.regVal}>
                  {regs[r].toString().padStart(3, " ")} (0x
                  {regs[r].toString(16).toUpperCase().padStart(2, "0")})
                </span>
              </div>
            ))}
          </div>
      ...
      <div style={S.blocksGrid}>
        ...
        <div style={S.block(activeBlock === "alu", "#51cf66")}>
          ...
          <div
            style={{
              ...S.blockVal,
              color: activeBlock === "alu" ? "#51cf66" : "#495057",
            }}
          >
            {activeBlock === "alu"
              ? (stageDetail.split("=").pop()?.trim() ?? "active")
              : "—"}
          </div>
        </div>
        ...
        <div
          style={{
            ...S.block(activeBlock === "memory", "#f06595"),
            gridColumn: "1 / -1",
          }}
        >
          <div style={S.blockTitle}>Memory (non-zero cells)</div>
          <div
            style={{ display: "flex", flexWrap: "wrap", gap: 8, fontSize: 12 }}
          >
            {memory
              .map((v, i) =>
                v !== 0 ? (
              ...
              .filter(Boolean)
              .slice(0, 24)}
            {memory.filter((v) => v !== 0).length === 0 && (
              <span style={{ color: "#495057" }}>empty</span>
            )}
          </div>
        </div>
      </div>
