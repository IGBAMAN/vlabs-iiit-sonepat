...
export function op74HC138_DEMUX(
  ...
  if (!isFullyDefined(e1) || !isFullyDefined(e2) || !isFullyDefined(e3))
    return all_x;
  if (e1[0] !== 1 || e2[0] !== 0 || e3[0] !== 0) return all_high;
  if (!isFullyDefined(a0) || !isFullyDefined(a1) || !isFullyDefined(a2))
    return all_x;

  const addr =
    (a2[0] as number) * 4 + (a1[0] as number) * 2 + (a0[0] as number);
  const out = { ...all_high };
...
export type Encoder8to3Outputs = {
  a0: Signal;
  a1: Signal;
  a2: Signal; // 3-bit code (active-low)
