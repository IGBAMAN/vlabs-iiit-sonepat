...
export const COLS         = 30;      // columns 1..30 (standard breadboard)
export const LONG_COLS    = 60;      // columns 1..60 (long / full-size breadboard)
...
// ── Helpers ───────────────────────────────────────────────────────────────────
export function colsForBoardType(type: string): number {
  return type === 'long-breadboard' ? LONG_COLS : COLS;
}

export function colsForBoardId(
  boardId: string,
  components: readonly { id: string; type: string }[],
): number {
  const board = components.find((c) => c.id === boardId);
  return board ? colsForBoardType(board.type) : COLS;
}

export function colToX(col: number, cols: number = COLS): number {
  return -(((cols - 1) / 2) * PITCH) + (col - 1) * PITCH;
}

/** Exact world position of the top of any tie-point hole. */
export function hole(col: number, row: string, cols: number = COLS): THREE.Vector3 {
  const z = Z[row.toLowerCase()];
  if (z === undefined) throw new Error(`Unknown row: "${row}"`);
  return new THREE.Vector3(colToX(col, cols), TOP_Y, z);
}
...
export function railHole(
  ...
  return new THREE.Vector3(colToX(col, cols), TOP_Y, Z[`rail_${rail}`]);
}
