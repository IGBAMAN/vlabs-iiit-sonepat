...
export const metadata = {
  ...
  description:
    "What the renderer expects — validation rules for circuit definitions.",
};
  ...
  return (
    <Prose>
      ...
      <p>
        ...
        <code> null</code> and are silently skipped rather than throwing. But
        circuits that violate these rules will look wrong or incomplete. Treat
      ...
      <ul>
        <li>
          Every <code>id</code> in <code>components[]</code> must be unique.
        </li>
        <li>
          Every <code>id</code> referenced in <code>show[]</code> or{" "}
          <code>highlight</code> must exist in <code>components[]</code>.
        </li>
        <li>
          Wire <code>from</code>/<code>to</code> that reference non-existent ids
          produce no wire (silent skip).
        </li>
      </ul>
      ...
      <ul>
        <li>
          Every LED must have a current-limiting resistor in series (330Ω
          typical).
        </li>
        <li>
          Resistor at col N → resistor p2 at col N+3 → LED anode at col N+2 (not
          N+4). The LED is placed 2 cols after the resistor start.
        </li>
      ...
      <ul>
        <li>
          Every wire must have a valid <code>from</code> and <code>to</code> —
          both must resolve to a position.
        </li>
        <li>
          Wires with unresolvable endpoints are silently dropped (no crash, but
          missing connections).
        </li>
        <li>
          Each ground return wire should use a unique <code>col</code> on the
          rail to avoid overlap.
        </li>
      </ul>
      ...
      <ul>
        <li>
          If present, <code>inputs</code> and <code>outputs</code> arrays must
          be non-empty.
        </li>
        <li>
          Every <code>row.inputs</code> key must be in the <code>inputs</code>{" "}
          array.
        </li>
        <li>
          Every <code>row.outputs</code> key must be in the <code>outputs</code>{" "}
          array.
        </li>
        <li>
          <code>activeInputs</code> keys in steps must match{" "}
          <code>truthTable.inputs</code> keys exactly.
        </li>
      </ul>
      ...
      <Callout $tone="tip">
        ...
        <p>
          Paste the generated circuit into Claude and ask it to check against
          this constraints list. It will catch column collisions, missing ground
          wires, and non-cumulative show arrays.
        </p>
      </Callout>

      <DocNav>
        <DocNavLink as={Link} href="/docs/types" data-dir="prev">
          TypeScript types
        </DocNavLink>
        <span />
      </DocNav>
